...
const Popular = () => {
    ...
    let allMovie = async () => {
        try {
            const data = await fetch(`https://api.themoviedb.org/3/movie/popular?api_key=1665407252326530351ed5a76c30158f&language=en-US&page=1`)
  ...
  return (
    ...
    {movie && movie.map((m) => (
        <div className='w-52 border-2 border-black p-1 flex flex-col items-center justify-center text-center' key={m.id}>
            <img src={`https://image.tmdb.org/t/p/w500/${m.poster_path}`} width="100%" className='border-black	border-2' key={m.id}></img>
